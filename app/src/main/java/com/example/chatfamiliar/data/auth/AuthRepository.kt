...
class AuthRepository {
    ...
    fun crearUsuario(correo: String,
                     password: String,
                     alCompletar: (Result<FirebaseUser>) -> Unit) {
    ...
    fun enviarCorreoVerificacion(alCompletar:
                                     (Result<Unit>) -> Unit) {
        ...
        usuario.sendEmailVerification()
            .addOnCompleteListener { tarea ->
                if (tarea.isSuccessful) {
                    ...
                    val excepcion = tarea.exception ?:
                    Exception("No se pudo enviar el correo " +
                            "de verificación.")
                    alCompletar(Result.failure(
                        excepcion))
                }
    ...
    fun comprobarCorreoVerificado(alCompletar:
                                      (Result<Boolean>)
                                  -> Unit) {
        val usuario = auth.currentUser
        if (usuario == null) {
        ...
        usuario.reload().addOnCompleteListener { tarea ->
                if (tarea.isSuccessful) {
                    ...
                    alCompletar(Result
                        .success(correoVerificado))
                } else {
                    val excepcion = tarea
                        .exception ?:
                        Exception("No se pudo " +
    ...
    fun iniciarSesion(correo: String,
                      password: String,
                      alCompletar: (Result<FirebaseUser>)
                      -> Unit) {
        auth.signInWithEmailAndPassword(correo,
            password).addOnCompleteListener { tarea ->
                if (tarea.isSuccessful) {
                    val usuario = tarea.result?.user
                    if (usuario != null) {
    ...
    fun cerrarSesion() {
        auth.signOut() }
    fun obtenerUsuarioActual(): FirebaseUser? { return auth
        .currentUser }
    fun obtenerCorreoActual(): String? { return auth
        .currentUser?.email }
}